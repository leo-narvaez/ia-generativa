...
def create_db():
    conn = sqlite3.connect('data/products.db')  # Nombre de la base de datos
    cursor = conn.cursor()
...
def save_to_db(details):
    ...
    cursor.execute('''
        INSERT INTO laptops (product_name, product_code, price, processor, ram, graphics, screen, battery)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
